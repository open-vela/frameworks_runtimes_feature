...
FeatureInstanceWamr::FeatureInstanceWamr(FeaturePrototype* proto, VTable vtable, int vtable_size)
    : FeatureInstance(proto, vtable, vtable_size)
    , instance_qjs_(new FeatureInstanceQjs(proto, vtable, vtable_size))
{
}

FeatureInstance* FeatureInstanceWamr::createInterface(VTable vtable, int vtable_size)
{
    // null param proto to be fixed
    return new FeatureInstanceWamr(nullptr, vtable, vtable_size);
}
...
bool FeatureInstanceWamr::removePromise(FtPromiseId pid)
{
    return instance_qjs_->removePromise(pid);
}
