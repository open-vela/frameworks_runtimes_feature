...
extern "C" int32_t get_array_struct_type(wasm_module_t wasm_module, int32_t array_type_idx,
            ...
            */
            ...
            for (int i = 0; i < rest_param_count; i++)
            {
                ...
                case WASM_ANYREF:
                {
                    ...
                    wasm_value_t tmp_val = { 0 };
                    /* call return_box_anyref api to box element as any */
                    wasm_anyref_obj_t any_obj = return_box_anyref(exec_env, dyntype_new_string(dyn_ctx, str, strlen(str)));
                    tmp_val.gc_obj = (wasm_obj_t)any_obj;
                    wasm_array_obj_set_elem(any_array, i, &tmp_val);
                }
                break;
