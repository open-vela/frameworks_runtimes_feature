...
int ClientConnection::createSession(const std::string& target) {
    ...
    if (android::getService<IMessageTransport>(android::String16(target.c_str()), &service) !=
        android::NO_ERROR) {
...
int ClientConnection::sendMessage(const std::string& target, const std::string& msg, int32_t pid_) {
    sp<IMessageTransport> service;
    if (android::getService<IMessageTransport>(android::String16(target.c_str()), &service) !=
        android::NO_ERROR) {
        ALOGE("ServiceManager can't find the service:%s", target.c_str());
...
void ClientConnection::sendBroadcast(const std::string& action, const std::string& data) {
    ActivityManager am;
    Intent intent;
...
Status MessageTransportServer::sendMessage(
        const ::std::string& message, const ::android::sp<::message_transport::IReply>& reply) {
...
void MessageTransportServer::serverReply(int reply_id, const std::string& message) {
...
void MessageTransportServer::sessionSend(int reply_id, const std::string& message) {
    if (reply_map_.find(reply_id) != reply_map_.end()) {
        reply_map_[reply_id]->onReply(message);
    }
}
