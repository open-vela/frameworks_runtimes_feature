...
public:
    MessageReply(int32_t id) : pid_(id), client_channel_cb_(nullptr), client_connection_(nullptr) {}
    Status onReply(const ::std::string& reply) override;
    // no use
    Status onSessionClose() override {
        return Status::ok();
...
public:
    ...
    int sendMessage(const std::string& target, const std::string& msg, int32_t pid) override;

    // override BroadcastChannel
    void sendBroadcast(const std::string& action, const std::string& data) override;
...
private:
    std::map<SessionId, sp<message_transport::IMessageTransport>> session_client_map_;
...
public:
    ...
    Status sendSessionMessage(const ::std::string& message,
                              const ::android::sp<::message_transport::IReply>& reply) override;
