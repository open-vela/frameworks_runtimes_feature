...
// setTimeout
JSValue __setTimeout(JSContext *ctx, JSValue this_val, int argc,
                     JSValue *argv) {
  if (argc < 2) {
...
static JSValue getRequireObject(feature_env_t *js_env,
                                ferry::FeatureManagerQjs *m) {
  JSValue func_data = JS_NewObject(js_env->ctx);
  JS_SetOpaque(func_data, m);
  JSValue ret =
      JS_NewCFunctionData(js_env->ctx, __require, 1, 0, 1, &func_data);
  JS_FreeValue(js_env->ctx, func_data);
  return ret;
}
...
int main(int argc, char **argv) {
  ...
  JSValue require = getRequireObject(&js_env, manager);
  JS_SetPropertyStr(js_env.ctx, global_obj, "require", require);

  JSValue setTimeout =
      JS_NewCFunction(js_env.ctx, __setTimeout, "setTimeout", 2);
  JS_SetPropertyStr(js_env.ctx, global_obj, "setTimeout", setTimeout);
  ...
  const char *test_content =
      ...
      "d);\n}\n\nfunction print(a) {\n    unittest.print(a);\n}\n";
  auto res = JS_Eval(js_env.ctx, test_content, strlen(test_content), "<eval>",
                     JS_EVAL_TYPE_GLOBAL);
  if (JS_IsException(res)) {
    const char *str = JS_ToCString(js_env.ctx, res);
    printf("test internal file error: %s\n", str);
    JS_FreeValue(js_env.ctx, res);
    return -1;
  }
  JS_FreeValue(js_env.ctx, res);
  // 加载 测试文件
  auto result = JS_Eval(js_env.ctx, js_str, strlen(js_str), "a.js",
